...
export const AdminView: React.FC<AdminViewProps> = ({
  ...
  const handleDeleteOrder = async (orderId: string) => {
    ...
    setLoading(true);
    try {
      // First archive to deleted_orders
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900">
      <div className="max-w-7xl mx-auto px-4 py-8">
        ...
        {activeTab === 'kitchen' ? (
          ...
          <SuperadminQueries orders={[...orders, ...deletedOrders]} categories={categories} burgers={burgers} />
                    ...
                    <tbody>
                      ...
                      {deletedOrders.slice(0, 50).map((order) => (
                        ...
                        >
                          ...
                          <td className="py-3"><span className="px-2 py-1 rounded bg-red-600 text-white text-xs font-bold">DELETED</span></td>
                          <td className="py-3">
                            <span className={`px-2 py-1 rounded text-xs ${
                              order.orderType === 'takeaway' ? 'bg-blue-600/20 text-blue-300' : 'bg-green-600/20 text-green-300'
                            }`}>
