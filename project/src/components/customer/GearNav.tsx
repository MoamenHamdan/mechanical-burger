...
export const GearNav: React.FC<GearNavProps> = ({ categories, activeCategoryId, onSelect }) => {
  ...
  const renderGear = (id: string, label: string) => {
    ...
    return (
      <button
        ...
        className={`relative w-24 h-24 rounded-full border transition-all duration-300 flex items-center justify-center select-none
      ...
      >
        <Settings
          ...
          strokeWidth={1.5}
        />
        <span className={`relative z-10 text-sm font-extrabold tracking-widest ${isActive ? 'text-orange-300' : 'text-gray-100'} drop-shadow-[0_0_8px_rgba(255,255,255,0.35)]`}>
          {label.toUpperCase()}
        </span>
      </button>
    );
  ...
  return (
    <div className="w-full flex flex-wrap items-center justify-center gap-6 py-6">
      {renderGear('all', 'All')}
