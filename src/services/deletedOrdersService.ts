...
// Service for managing deleted orders
export const deletedOrdersService = {
  ...
  async create(order: Order): Promise<string> {
    try {
      ...
      console.error('Error creating deleted order record:', error);
      throw new Error('Failed to create deleted order record');
    }
  },
  ...
  subscribe(callback: (orders: Order[]) => void) {
    return onSnapshot(
      ...
      (error) => {
        console.error('Error in deleted orders subscription:', error);
      }
    );
  }
};